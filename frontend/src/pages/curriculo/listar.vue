...
<template>
  <div class="w-3/4 mx-auto p-6 bg-white shadow-md rounded-md mt-10 border border-gray-200">
    ...
    <table v-if="curriculos.length && !loading" class="w-full table-auto border-collapse">
      ...
      <tbody>
        <tr v-for="curriculo in curriculos" :key="curriculo.id">
          <td class="border px-4 py-2">{{ curriculo.id }}</td>
          ...
          <td class="border px-4 py-2">{{ formatarDataHora(curriculo.dataCriado) }}</td>
